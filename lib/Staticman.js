...
Staticman.prototype._checkBlacklist = function (fields) {
	  // return Promise.resolve(fields)

	  const blacklist = this.siteConfig.get('blacklist')
	  
	  if (!blacklist) return Promise.resolve(fields)
	/*
	  Object.keys(blacklist).forEach(field => {
	      if (fields[field]) {
	        const blacklistItems = [].concat(blacklist[field])

	        blacklistItems.forEach(blacklistItem => {
				  blacklistItem.forEach(bl => {
					  if (fields[blacklistItem] === bl) {
						  return Promise.reject(errorHandler('BLACKLISTED'))
					  }
				  })
	        })
	      }
...
Staticman.prototype.processEntry = function (fields, options) {
  ...
  return this.getSiteConfig().then(config => {
    ...
    return this._checkBlacklist(fields)
  }).then(fields => {
    // Validate fields
